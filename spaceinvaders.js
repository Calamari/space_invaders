(function(win, doc, $) {
  ...
  var Game = function(canvas, config) {
    var MAX_PIXEL     = 12,
        IS_MOBILE     = win.DeviceOrientationEvent,
        ...
        enemyShootProbability = 0.075,
        incEnemyShootProb     = 0.05,
    ...
    function loop() {
      ...
      if (mothership) {
        ...
        if ((mothershipCount <= level || ended) && lastMothershipTime < loopTime - config.mothership.minTime*1000 && Math.random() < config.mothership.probability) {
          mothershipMovingRight = Math.random() < 0.5;
